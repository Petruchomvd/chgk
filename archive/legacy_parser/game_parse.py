...
def _extract_game_data(game_number: int, url: str, soup: BeautifulSoup) -> Optional[Dict[str, Any]]:
    ...
    game_start_date = parse_russian_date(start_raw) if start_raw else None
    game_end_date = parse_russian_date(end_raw) if end_raw else None
    game_published_date = parse_russian_date(published_raw) if published_raw else None
