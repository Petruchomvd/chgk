...
# Слова, которые natasha ошибочно тегает как ORG (местоимения, стоп-слова и т.д.)
ORG_DENYLIST = {
    "ее", "её", "она", "его", "им", "их", "альфа", "оно", "первый",
    "это", "они", "нем", "ней", "них", "ним", "ними", "этого",
    "этим", "икса", "ему", "иксом", "он", "альф", "этом", "ею",
    "там", "иксов", "вторым", "второй", "альфу", "иксы", "альфе",
    "ими", "иксе", "первым", "первая", "бета", "альфой",
    "таким", "нее", "неё", "сми",
}
...
def is_org_like_entity(name: str) -> bool:
    ...
    # Отсечь местоимения и стоп-слова, которые natasha ошибочно тегает как ORG
    if lower in ORG_DENYLIST:
        return False

    if any(k in lower for k in ORG_KEYWORDS):
        return True

    # Аббревиатуры типа ООН, НАТО, МКС, NASA (но не короткие русские слова)
    if re.fullmatch(r"[A-ZА-ЯЁ0-9]{2,8}", text):
        # Дополнительная проверка: если слово есть в общих стоп-словах, отклоняем
        if lower in ALL_STOPWORDS:
            return False
        return True
